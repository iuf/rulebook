\chapter{Freestyle \& Standard Skills \label{chap:freestyle} Overview}
...
\textbf{Props} are all items which are used by the rider in his/her performance and require a technical handling by the rider (e.g. typical objects from RSG, like clubs, bands). 
...
For Junior Expert and Expert categories at Unicon, it is forbidden to use too large of Decorations (including people) that the competitor cannot carry and/or put on by oneself. 

For Props and Decoration neither fire nor sharp objects (i.e. juggling knives) are allowed.
...
\subsection{Props}
Props are all items which are used by the rider in his/her performance and require a technical handling by the rider. 
These items can be used to do a unicycling trick, like rope skipping with the unicycle.
However, they can also be employed to show a non-unicycling skill which supports and enhances the choreography, like the elaborate use of a hat.
Props have to be presented by the rider.
It is not mandatory to include props in the performance.
...
In contrast to props, decoration is used to present the rider or clarify the theme of the performance.
Decoration does not require a technical handling by the rider.
For example other persons in costumes and background images.
Decoration is no personal contribution of the rider and therefore effects of the Decorations should not be judged.
On the contrary, scenery can also be judged negatively if it distracts from the rider’s performance.
...
Judging for Flatland and the Street Comp is described in part \ref{part:flat-street}.
...
Only `unicycling skills' will be counted (see definition in section \ref{sec:general_definitions}).
...
\textbf{Props:} Props and Decorations are judged according to their definitions in section \ref{sec:freestyle_freestyle-rules_individual-freestyle-overview_props-and-decorations}.
